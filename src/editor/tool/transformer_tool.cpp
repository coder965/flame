...
static const ImU32 translation_line_color = 0xAAAAAAAA;
static const char *translation_info_mask[] = { "X : %5.3f", "Y : %5.3f", "Z : %5.3f", "X : %5.3f Y : %5.3f", "Y : %5.3f Z : %5.3f", "X : %5.3f Z : %5.3f", "X : %5.3f Y : %5.3f Z : %5.3f" };
static const char *rotation_info_mask[] = { "X : %5.2f deg %5.2f rad", "Y : %5.2f deg %5.2f rad", "Z : %5.2f deg %5.2f rad", "Screen : %5.2f deg %5.2f rad" };
static const char *scale_info_mask[] = { "X : %5.2f", "Y : %5.2f", "Z : %5.2f", "XYZ : %5.2f" };
static const int translation_info_index[] = { 0,0,0, 1,0,0, 2,0,0, 0,1,0, 1,2,0, 0,2,1, 0,1,2 };
...
static const float snap_tension = 0.5f;
	...
	switch (operation)
	{
		case TRANSLATE:
		{
			...
			if (using_)
			{
				...
				ImFormatString(tmps, sizeof(tmps), translation_info_mask[type - MOVE_X], delta_info[translation_info_index[component_info_index]], delta_info[translation_info_index[component_info_index + 1]], delta_info[translation_info_index[component_info_index + 2]]);
