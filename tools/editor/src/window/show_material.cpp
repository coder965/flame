...
void show_material(flame::Material *m)
{
	...
	show_map(std::bind(&flame::Material::get_albedo_alpha_map_name, m), "albedo(rgb) alpha(a) map, [right click] to remove", std::bind(&flame::Material::set_albedo_alpha_map, m, std::placeholders::_1));
	ImGui::SameLine();
	...
	show_map(std::bind(&flame::Material::get_spec_roughness_map_name, m), "spec(r) roughness(g) map, [right click] to remove", std::bind(&flame::Material::set_spec_roughness_map, m, std::placeholders::_1));
	ImGui::SameLine();
	...
	show_map(std::bind(&flame::Material::get_normal_height_map_name, m), "normal(rgb) height(a) map, [right click] to remove", std::bind(&flame::Material::set_normal_height_map, m, std::placeholders::_1));
	ImGui::SameLine();
