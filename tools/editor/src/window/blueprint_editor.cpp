...
BlueprintEditor::Node::Node(int id, const char* name, const ImVec2& pos, float value, const ImVec4& color, int inputs_count, int outputs_count)
{ 
...
ImVec2 BlueprintEditor::Node::GetInputSlotPos(int slot_no) const
{ 
	return ImVec2(Pos.x, Pos.y + Size.y * ((float)slot_no + 1) / ((float)InputsCount + 1)); 
}

ImVec2 BlueprintEditor::Node::GetOutputSlotPos(int slot_no) const
{ 
	return ImVec2(Pos.x + Size.x, Pos.y + Size.y * ((float)slot_no + 1) / ((float)OutputsCount + 1)); 
}

BlueprintEditor::NodeLink::NodeLink(int input_idx, int input_slot, int output_idx, int output_slot)
{
	InputIdx = input_idx; 
...
void BlueprintEditor::on_show()
{
	...
	for (int node_idx = 0; node_idx < nodes.Size; node_idx++)
	{
		...
		draw_list->AddRectFilled(node_rect_min, node_rect_min + node->Size, node_bg_color, 4.0f);
		draw_list->AddRect(node_rect_min, node_rect_min + node->Size, IM_COL32(100, 100, 100, 255), 4.0f);
