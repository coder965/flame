...
int main(int argc, char **args)
{
	...
	sm->run([&](){
		...
		ui->text("score: %d", score);
		static vec3 coord = vec3(0.f);
		static vec3 force = vec3(0.f);
		ui->dragfloat("x", &coord.x, 0.1f);
		ui->dragfloat("y", &coord.y, 0.1f);
		ui->dragfloat("z", &coord.z, 0.1f);
		ui->dragfloat("fx", &force.x, 0.1f);
		ui->dragfloat("fy", &force.y, 0.1f);
		ui->dragfloat("fz", &force.z, 0.1f);
		if (ui->button("Throw A Ball"))
		{
			Ins i;
			i.coord = coord;
			i.quat = vec4(0.f, 0.f, 0.f, 1.f);
			i.size = vec3(0.1f);
			i.create(p_d, true, material, scene);
			i.r->add_force(force);
			inses.push_back(i);
